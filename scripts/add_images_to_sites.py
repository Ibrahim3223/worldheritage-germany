"""
Add multiple image URLs to existing site markdown files
Fetches images from Wikimedia Commons using Wikidata IDs
"""
...
def main():
    """Process all site markdown files"""
    ...
    print(f"[*] Found {len(md_files)} site files")
        ...
        try:
            ...
            if new_images:
                ...
                if update_markdown_images(file_path, new_images):
                    updated_count += 1
                    print(f"[{i}/{len(md_files)}] Updated: {site_name} ({len(new_images)} images)")
                else:
                    skipped_count += 1
            else:
                skipped_count += 1
